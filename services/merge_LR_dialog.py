...
def _time_from_fields(obj: Dict[str, Any]) -> Tuple[float, float]:
    ...
    start = _first_not_none(
        ...
        # 'Offset'이 ticks일 수도 sec일 수도 있음(heuristic)
...
def parse_json_atoms(
    ...
    # 단어 토큰 선택 우선순위 (text_field에 따라)
    def _pick_token(w: Dict[str, Any]) -> Optional[str]:
        if text_field == "display":
            return (w.get("displayText") or w.get("word") or w.get("text") or
                    w.get("itn") or w.get("maskedITN"))
        if text_field == "lexical":
            return (w.get("word") or w.get("text") or w.get("displayText") or
                    w.get("itn") or w.get("maskedITN"))
        if text_field == "maskedITN":
            return (w.get("maskedITN") or w.get("itn") or w.get("displayText") or
                    w.get("word") or w.get("text"))
        # default: itn
        return (w.get("itn") or w.get("maskedITN") or w.get("displayText") or
                w.get("word") or w.get("text"))
    ...
    for ph in phrases:
        ...
        if isinstance(words, list) and words:
            ...
            def flush():
                ...
                if seg_tokens:
                    out.append(Atom(seg_start, last_end or seg_start, default_speaker, " ".join(seg_tokens).strip()))
        ...
        # 문장 레벨 텍스트: text_field 우선
        text = None
        if hyp0:
            text = (hyp0.get(text_field)
                    or (text_field != "display" and hyp0.get("display"))
                    or hyp0.get("lexical")
                    or hyp0.get("itn")
                    or hyp0.get("maskedITN"))
        if not text:
            text = (ph.get(text_field)
                    or (text_field != "display" and ph.get("display"))
                    or ph.get("lexical")
                    or ph.get("itn")
                    or ph.get("maskedITN")
                    or ph.get("text"))
...
#                 token = (w.get("itn") or w.get("ITN") or w.get("maskedITN")
#                        or w.get("displayText") or w.get("word") or w.get("text"))
#                 if not token:
#                     continue
#                 token = str(token).strip()
#                 ws, we = _time_from_fields(w)
...
def main():
    ...
    # ✅ 기본 출력 디렉토리 추가
    ap.add_argument("--outdir", default=".", help="기본 출력 디렉토리(상대/절대 경로 모두 가능)")
    ...
    ap.add_argument("--text-field",
                    choices=["display", "itn", "lexical", "maskedITN"],
                    default="itn",
                    help="문장 레벨 텍스트 필드 선택(숫자 표기는 itn 권장)")
    ap.add_argument("--single-line-if-noswitch",
                    action="store_true",
                    help="화자 전환이 0회면 한 줄로 합치기")
